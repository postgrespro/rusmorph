    ...
    // check for overflow
      {
        ...
        if ( (cchout = win2client(pforms, eforms - pforms,
          (const char*)fmbuff, ccstem )) == (size_t)-1 )  return LEMMBUFF_FAILED;
        else pforms += cchout + 1;
      }
    }
...
// doBuildForm implementation
  ...
  // flexion
    ...
    // output the string
      if ( (cbcopy = win2client(output, outend - output, (const char*)szform, outptr - szform )) == (size_t)-1 )
